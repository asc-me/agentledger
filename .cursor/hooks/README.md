# Graphban Cursor hooks (AL-214 / PRD-11 §D3)

`.cursor/hooks.json` wires Cursor's agent lifecycle to the Graphban loop — MCP
...
| Hook | Handler | Effect |
| --- | --- | --- |
| `sessionStart`  | `gb_session_start.py`  | Injects the operating-loop primer as `additional_context` (+ a live `get_context` snapshot when `GRAPHBAN_MCP_URL`/`GRAPHBAN_API_KEY` are set). |
| `afterFileEdit` | `gb_after_file_edit.py`| **Warns** when an edited file is outside the claimed item's touchpoints. |
| `stop`          | `gb_stop.py`           | Best-effort `followup_message` nudging `update_item(review)` + `extract_lessons`. |
...
`gb_after_file_edit.py` reads the current claim from `GRAPHBAN_CLAIM_FILE`
(default `.cursor/graphban-claim.json`, git-ignored). Shape — see
[`../graphban-claim.example.json`](../graphban-claim.example.json):
...
| Var | Used by | Purpose |
| --- | --- | --- |
| `GRAPHBAN_MCP_URL` + `GRAPHBAN_API_KEY` | `gb_session_start.py` | Optional live `get_context` enrichment (fail-open). |
| `GRAPHBAN_CLAIM_FILE` | `gb_after_file_edit.py` | Override the claim-manifest path. |
