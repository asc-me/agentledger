...
def create_request(body: RequestCreate, db: Session = Depends(get_db), _: User = Depends(get_current_user)):
    try:
...
def vote(request_id: str, body: RequestVoteIn, db: Session = Depends(get_db), _: User = Depends(get_current_user)):
...
def link(request_id: str, body: RequestLinkIn, db: Session = Depends(get_db), _: User = Depends(get_current_user)):
    try:
