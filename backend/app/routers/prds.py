...
def update_prd(prd_id: str, body: PrdUpdate, db: Session = Depends(get_db), _: User = Depends(get_current_user)):
    try:
...
def snapshot(prd_id: str, body: PrdVersionIn, db: Session = Depends(get_db), _: User = Depends(get_current_user)):
...
def link(prd_id: str, body: PrdLinkIn, db: Session = Depends(get_db), _: User = Depends(get_current_user)):
...
def ai(prd_id: str, body: PrdAiIn, db: Session = Depends(get_db), _: User = Depends(get_current_user)):
    try:
